...
import CommercialPropertyGridClient from "./CommercialPropertyGridClient";

/* export const dynamic = "force-dynamic"; */
export const revalidate = 60;
export const metadata = {
    title: "Commercial Properties | Sangli Properties",
    description:
        "Explore commercial properties in Sangli & Miraj – shops, offices, showrooms and more.",
};
...
export default async function CommercialPropertiesPage({ searchParams }: PageProps) {
    ...
    const crmProperties = await getWebsiteProperties({ type: "commercial" });

    const commercialProperties = crmProperties.filter((p: CrmWebsiteProperty) => {
        ...
        <main className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)]">
            ...
            <section className="relative overflow-hidden border-b border-[var(--color-border)] bg-[var(--color-header)] text-white">
                ...
                <div className="absolute top-[5px] bottom-[5px] left-[370px] right-[20px] hidden md:flex items-center justify-end gap-4 overflow-hidden">
                    ...
                    <img
                        src="/Commerical1.png"
                        alt="Commercial Advertisement 1"
                        className="h-full w-[28%] -translate-y-[18px] rounded-xl object-cover object-center"
                    />

                    <img
                        src="/Commerical2.png"
                        alt="Commercial Advertisement 2"
            ...
            <section className="bg-[var(--color-bg)]">
                <div className="mx-auto max-w-[1400px] px-4 py-10 sm:py-12">
                    ...
                    {/* ✅ Same filters (purpose/search/min-max area/budget) */}
                    <ResidentialFilters searchPlaceholder="Search by title, location… (Flat, Showroom, Office, VijayNagar)" />

                    {/* GRID */}
                    <CommercialPropertyGridClient properties={commercialProperties} />
                </div>
            </section>
        </main>
    );
