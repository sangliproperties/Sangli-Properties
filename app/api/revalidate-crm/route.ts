...
export async function POST(req: NextRequest) {
    try {
        ...
        // Global list caches
        revalidateTag("crm-properties");
        revalidateTag("crm-properties-all");

        // Type-specific list caches
        if (propertyType) {
            revalidateTag(`crm-properties-type-${propertyType}`);
        } else {
            // Safe fallback if CRM does not send type
            revalidateTag("crm-properties-type-residential");
            revalidateTag("crm-properties-type-commercial");
            revalidateTag("crm-properties-type-industrial");
            revalidateTag("crm-properties-type-land");
        }

        // Single property detail cache
        if (propertyId) {
            revalidateTag(`crm-property-${propertyId}`);
